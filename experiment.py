...
if __name__ == '__main__':
    ...
    #     ts = datetime.timestamp(dt)
    #     # getting the timestamp
    #     tmp.update({"centralized":False, "l_lr":lr, "hybrid_loss_weights":weight_comb, "weighting_scheme":"FEDAVG"})
    #     networks_config.append(tmp)
    #     networks_name.append(f"{ts}_{experience_name}_FedAvg_lr{lr}_batch{tmp['batch_size']}_epoch{tmp['g_epoch']*tmp['l_epoch']}_lambdas{str(tmp['hybrid_loss_weights'][0])}_{str(tmp['hybrid_loss_weights'][1])}")
