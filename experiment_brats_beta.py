...
def runExperiment(datapath, num_repetitions, networks_config, networks_name, exp_name=None, modality="ADC",
    ...
    partitions_paths, partitions_paths_add_mod,_,_ = dataPreprocessing(datapath, modality, clients, additional_modalities, nested, multi_label)
...
if __name__ == '__main__':
    path = 'fedem_ISLE22/'
    root_data = '/str/data/ASAP/BRATS_2019_ubelix/center_wise/'

    experiment_name = "brats_beta_no_transfo_Blob_DiceLossCE"
    modality="stacked"

    clients=["BraTS19_2013_","BraTS19_CBICA_","BraTS19_TCIA0_","BraTS19_TMC_"]
    ...
    default = {"g_epoch":300,#10
               "l_epoch":2,#30
               "g_lr":0.01,
               "l_lr":0.0001,
               "K":len(clients),
               "clients":clients,
               "suffix":"brats_beta_tio_",
               "nn_class": "segresnet",
               "space_cardinality":3,
               ...
               "clamp_max":4000,
               "patch_size":(96,96,32),
               "padding":(48,48,16), #typically half the dimensions of the patch_size
               "max_queue_length":16,
               "patches_per_volume":4,
               "loss_fun":"diceloss", #"dicelossCE", #diceloss_CE
    ...
    #for lr in [0.00994, 0.0116]:
        tmp = default.copy()
        tmp.update({"l_lr":lr, "weighting_scheme": "BETA", "beta_val":0.99})
        networks_config.append(tmp)

        networks_name.append(f"{experiment_name}_federated_lr{lr}_batch{tmp['batch_size']}_epoch{tmp['g_epoch']*tmp['l_epoch']}")
        #legacy network naming, no lambdas (valid for v1 to v4)
        #networks_name.append(f"{experiment_name}_CENTRALIZED_lr{lr}_batch{tmp['batch_size']}_epoch{tmp['g_epoch']*tmp['l_epoch']}")
    ...
    valid_metrics, test_metrics = runExperiment(datapath=root_data,
                                                num_repetitions=4,
